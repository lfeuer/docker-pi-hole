...
| `DNS1: <IP>`<br/> *Optional, Deprecated* *Default: 8.8.8.8* | Primary upstream DNS provider, default is google DNS. Use `PIHOLE_DNS_x` instead. 
| `DNS2: <IP>`<br/> *Optional, Deprecated* *Default: 8.8.4.4* | Secondary upstream DNS provider, default is google DNS, `no` if only one DNS should used
| `PIHOLE_DNS_1: <IP>`<br/> *Optional* *Default: 8.8.8.8* | Primary upstream DNS provider, default is google DNS
| `PIHOLE_DNS_2: <IP>`<br/> *Optional* *Default: 8.8.4.4* | Secondary upstream DNS provider, default is google DNS, `no` if only one DNS should used
| `PIHOLE_DNS_3: <IP>`<br/> *Optional* | Primary upstream IPv6-DNS provider
| `PIHOLE_DNS_4: <IP>`<br/> *Optional* | Secondary upstream IPv6-DNS provider
...
| `REV_SERVER: <"true"\|"false">`<br/> *Optional* *Default: "false"* | Enable DNS conditional forwarding for device name resolution. If set, any `CONDITIONAL_FORWARDING_*` will be ignored. 
...
| `REV_SERVER_CIDR: <Network CIDR>`<br/> *Optional* | If conditional forwarding is enabled, set the CIDR of the local network, eg: `192.168.0.0/24`
| `CONDITIONAL_FORWARDING: <"true"\|"false">`<br/> *Optional, Deprecated* *Default: "false"* | Enable DNS conditional forwarding for device name resolution
| `CONDITIONAL_FORWARDING_IP: <Router's IP>`<br/> *Optional, Deprecated* | If conditional forwarding is enabled, set the IP of the local network router
| `CONDITIONAL_FORWARDING_DOMAIN: <Network Domain>`<br/> *Optional, Deprecated* | If conditional forwarding is enabled, set the domain of the local network router
| `CONDITIONAL_FORWARDING_REVERSE: <Reverse DNS>`<br/> *Optional, Deprecated* | If conditional forwarding is enabled, set the reverse DNS of the local network router (e.g. `0.168.192.in-addr.arpa`)
